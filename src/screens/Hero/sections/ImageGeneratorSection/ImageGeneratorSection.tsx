  ...
  return (
    <section className="flex flex-col w-full items-center justify-center gap-14 px-16 py-[72px] relative bg-[#ffffff0a]">
      ...
      <div className="flex flex-col items-center gap-8 relative self-stretch w-full">
        <Tabs
          defaultValue="all"
          className="p-1.5 bg-[#ffffff1f] rounded-[46px] border border-solid border-[#0000000d]"
        >
          <TabsList className="bg-transparent gap-0 h-auto p-0">
            {tabItems.map((item) => (
              <TabsTrigger
                key={item.value}
                value={item.value}
                className={`gap-2 px-4 py-1 rounded-[42px] font-display-XS font-[number:var(--display-XS-font-weight)] text-[length:var(--display-XS-font-size)] text-center tracking-[var(--display-XS-letter-spacing)] leading-[var(--display-XS-line-height)] whitespace-nowrap [font-style:var(--display-XS-font-style)] data-[state=active]:bg-white data-[state=active]:text-[#191124] data-[state=inactive]:text-white data-[state=inactive]:opacity-60 h-auto`}
              >
                {item.label}
              </TabsTrigger>
            ))}
        ...
        <Button className="inline-flex px-8 py-4 overflow-hidden border-2 border-solid border-[#ffffff33] bg-[linear-gradient(270deg,rgba(225,183,75,1)_0%,rgba(232,65,99,1)_30%,rgba(166,4,255,1)_62%,rgba(100,0,255,1)_84%,rgba(62,8,224,1)_100%),linear-gradient(0deg,rgba(255,255,255,1)_0%,rgba(255,255,255,1)_100%)] items-center justify-center gap-2 rounded-xl font-display-XS font-[number:var(--display-XS-font-weight)] text-neutral-0 text-[length:var(--display-XS-font-size)] text-center tracking-[var(--display-XS-letter-spacing)] leading-[var(--display-XS-line-height)] whitespace-nowrap [font-style:var(--display-XS-font-style)] h-auto hover:opacity-90">
          MAKE YOUR OWN VIDEO
        </Button>
      </div>
    </section>
